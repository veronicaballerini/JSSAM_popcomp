...
for(s in 1:nsam){
  ...
  M<-NULL
  colors<-matrix(truecolors[s,],nrow=numcol,ncol=1)
  p<-round(numMFNCHypergeo(colors,sum(colors),100000, true.w))/100000
  M<-rbind(M, 
           round(c(lambda_M1,
             lambda_M1*p[2]/p[1],
             lambda_M1*p[3]/p[1],
  ...
  epsilon[1]<-quantile(abs(x1/sum(colors)-truefreq[1]),0.025)
  ...
  epsilon[2]<-quantile(abs(x2/sum(colors)-truefreq[2]),0.025)
  ...
  epsilon[3]<-quantile(abs(x3/sum(colors)-truefreq),0.025)
  ...
  epsilon[4]<-quantile(abs(x4/sum(colors)-truefreq),0.025)
  ...
  epsilon[5]<-quantile(abs(x5/sum(colors)-truefreq),0.025)
  ...
  for(iter in 2:niter){
    ...
    if(iter%%100==0){
      print(paste("SAMPLE N°", s, "ITER N°", iter))
    }
