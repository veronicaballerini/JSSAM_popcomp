sensitivity_step<-function(year,
                           gender,
                           data=datalong,
                           niter=50000,
                           burnin=30000,
  ...
  w2<-read.table(paste(wd,"/output/step2_",disciplines[i],gender,"sdw1.csv",sep=""),
                 sep=",",
                 header=TRUE)
  w2<-w2[,4]
  ...
  write.csv(save,paste(wd,"/output/sensitivity_",year,disciplines[i],gender,qnt,".csv",sep=""))
  
  print(c(accM/(niter-burnin),
          accw/(niter-burnin),
          hdi(Mthin),
          hdi(wthin)))
